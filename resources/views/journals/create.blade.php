...
.btn-remove-line { visibility: hidden; }
.line-row:hover .btn-remove-line { visibility: visible; }
</style>
@endpush
@section('content')
<div class="card">
    <div class="card-header"><i class="bi bi-plus-circle me-2"></i>Buat Jurnal Umum</div>
    <div class="card-body">
                ...
                <table class="table table-bordered" id="linesTable">
                    ...
                    @if(old('lines'))
                        ...
                        <tr class="line-row">
                            <td><select name="lines[{{ $i }}][account_id]" class="form-select form-select-sm" required>
                                ...
                                <option value="{{ $acc->id }}" @selected($acc->id == $line['account_id'])>{{ $acc->account_code }} - {{ $acc->name }}</option>
                                @endforeach
                            </select></td>
                            <td><input type="text" name="lines[{{ $i }}][description]" class="form-control form-control-sm" value="{{ $line['description'] ?? '' }}"></td>
                            <td><input type="number" name="lines[{{ $i }}][debit]" class="form-control form-control-sm text-end line-debit" value="{{ $line['debit'] ?? 0 }}" min="0" step="1"></td>
                            <td><input type="number" name="lines[{{ $i }}][credit]" class="form-control form-control-sm text-end line-credit" value="{{ $line['credit'] ?? 0 }}" min="0" step="1"></td>
                    ...
                    @else
                        <tr class="line-row">
                            ...
                            <td><input type="number" name="lines[0][debit]" class="form-control form-control-sm text-end line-debit" value="0" min="0" step="1"></td>
                            <td><input type="number" name="lines[0][credit]" class="form-control form-control-sm text-end line-credit" value="0" min="0" step="1"></td>
                            <td><button type="button" class="btn btn-sm btn-outline-danger btn-remove-line" onclick="removeLine(this)"><i class="bi bi-x"></i></button></td>
                        </tr>
                        <tr class="line-row">
                            ...
                            <td><input type="number" name="lines[1][debit]" class="form-control form-control-sm text-end line-debit" value="0" min="0" step="1"></td>
                            <td><input type="number" name="lines[1][credit]" class="form-control form-control-sm text-end line-credit" value="0" min="0" step="1"></td>
                            <td><button type="button" class="btn btn-sm btn-outline-danger btn-remove-line" onclick="removeLine(this)"><i class="bi bi-x"></i></button></td>
                        </tr>
                    @endif
                    </tbody>
...
function addLine() {
    ...
    tr.innerHTML = `
        <td><select name="lines[${lineIndex}][account_id]" class="form-select form-select-sm" required>
            <option value="">— Pilih Akun —</option>${accountOptions}</select></td>
        <td><input type="text" name="lines[${lineIndex}][description]" class="form-control form-control-sm"></td>
        <td><input type="number" name="lines[${lineIndex}][debit]" class="form-control form-control-sm text-end line-debit" value="0" min="0" step="1"></td>
        <td><input type="number" name="lines[${lineIndex}][credit]" class="form-control form-control-sm text-end line-credit" value="0" min="0" step="1"></td>
        <td><button type="button" class="btn btn-sm btn-outline-danger btn-remove-line" onclick="removeLine(this)"><i class="bi bi-x"></i></button></td>
    `;
...
function updateTotals() {
    let d = 0, c = 0;
    document.querySelectorAll('.line-debit').forEach(i => d += parseFloat(i.value)||0);
    document.querySelectorAll('.line-credit').forEach(i => c += parseFloat(i.value)||0);
    ...
    if (Math.abs(d - c) < 0.01 && d > 0) {
        ind.innerHTML = '<span class="text-success"><i class="bi bi-check-circle me-1"></i>Seimbang</span>';
    } else {
        ind.innerHTML = `<span class="text-danger"><i class="bi bi-exclamation-circle me-1"></i>Selisih: ${(d-c).toLocaleString('id-ID')}</span>`;
    }
}

function attachListeners() {
    document.querySelectorAll('.line-debit,.line-credit').forEach(i => {
        i.removeEventListener('input', updateTotals);
