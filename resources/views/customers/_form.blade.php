...
<input type="email" name="email" class="form-control" value="{{ old('email', $customer->email ?? '') }}"></div>
...
<div class="mb-3"><label class="form-label">Akun Piutang</label>
<select name="ar_account_id" class="form-select">
<option value="">— Default —</option>
@foreach($accounts as $a)<option value="{{ $a->id }}" @selected(old('ar_account_id', $customer->ar_account_id ?? '') == $a->id)>{{ $a->account_code }} - {{ $a->name }}</option>@endforeach
</select></div>
<div class="form-check"><input type="hidden" name="is_active" value="0">
<input type="checkbox" name="is_active" value="1" class="form-check-input" id="ca" @checked(old('is_active', $customer->is_active ?? true))>
<label class="form-check-label" for="ca">Aktif</label></div>
