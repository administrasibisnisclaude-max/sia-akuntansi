...
@section('content')
<div class="card">
    <div class="card-header"><i class="bi bi-pencil me-2"></i>Edit Tagihan: {{ $bill->bill_number }}</div>
    <div class="card-body">
        <form method="POST" action="{{ route('ap.bills.update', $bill) }}">
            @csrf @method('PUT')
            <div class="row g-3 mb-4">
                <div class="col-md-4">
                    <label class="form-label">Vendor <span class="text-danger">*</span></label>
                    <select name="vendor_id" class="form-select" required>
                        @foreach($vendors as $v)<option value="{{ $v->id }}" @selected($v->id == $bill->vendor_id)>{{ $v->name }}</option>@endforeach
                    </select>
                </div>
                <div class="col-md-3"><label class="form-label">Tanggal</label>
                <input type="date" name="date" class="form-control" value="{{ $bill->date->format('Y-m-d') }}" required></div>
                <div class="col-md-3"><label class="form-label">Jatuh Tempo</label>
                <input type="date" name="due_date" class="form-control" value="{{ $bill->due_date->format('Y-m-d') }}" required></div>
            </div>
            <div class="table-responsive mb-3">
                <table class="table table-bordered">
                    ...
                    <tr class="line-row">
                        <td><select name="lines[{{ $i }}][item_id]" class="form-select form-select-sm item-select"><option value="">—</option>
                            @foreach($items as $item)<option value="{{ $item->id }}" @selected($item->id == $line->item_id) data-price="{{ $item->buy_price }}">{{ $item->name }}</option>@endforeach
                        </select></td>
            ...
            <div class="mb-3" style="max-width:400px">
                ...
                <textarea name="notes" class="form-control" rows="2">{{ $bill->notes }}</textarea>
            </div>
            <div class="d-flex gap-2">
                <button class="btn btn-primary">Perbarui</button>
                <a href="{{ route('ap.bills.show', $bill) }}" class="btn btn-secondary">Batal</a>
            </div>
        </form>
    </div>
</div>
...
@push('scripts')
<script>
const itemData = { @foreach($items as $item){{ $item->id }}: { price: {{ $item->buy_price }}, name: '{{ addslashes($item->name) }}' },@endforeach };
const itemOptions = `@foreach($items as $item)<option value="{{ $item->id }}" data-price="{{ $item->buy_price }}">{{ $item->name }}</option>@endforeach`;
let idx = {{ $bill->lines->count() }};
