@extends('layouts.app')
@section('title', 'Buat Tagihan Pembelian')
@section('content')
<div class="card">
    <div class="card-header"><i class="bi bi-plus-circle me-2"></i>Buat Tagihan Pembelian</div>
    <div class="card-body">
        <form method="POST" action="{{ route('ap.bills.store') }}">
            @csrf
                ...
                <table class="table table-bordered">
                    ...
                    <tr class="line-row">
                        <td><select name="lines[0][item_id]" class="form-select form-select-sm item-select">
                            ...
                            @foreach($items as $item)<option value="{{ $item->id }}" data-price="{{ $item->buy_price }}">{{ $item->name }}</option>@endforeach
                        </select></td>
                        ...
                        <td><input type="number" name="lines[0][tax_rate]" class="form-control form-control-sm text-end line-tax" value="0" min="0" max="100"></td>
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('ap.bills.index') }}" class="btn btn-secondary">Batal</a>
            </div>
        </form>
    </div>
</div>
...
function attachRow(tr){tr.querySelector('.item-select')?.addEventListener('change',function(){const id=this.value;if(id&&itemData[id]){tr.querySelector('.line-price').value=itemData[id].price;if(!tr.querySelector('[name*="description"]').value)tr.querySelector('[name*="description"]').value=itemData[id].name;calcRow(tr);}});tr.querySelectorAll('.line-qty,.line-price').forEach(i=>i.addEventListener('input',()=>calcRow(tr)));}
