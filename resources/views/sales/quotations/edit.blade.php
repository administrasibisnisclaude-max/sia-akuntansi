@extends('layouts.app')
@section('title', 'Edit Penawaran Harga')
@push('styles')
<style>.line-row td{vertical-align:middle;}</style>
@endpush
@section('content')
<div class="card">
    <div class="card-header"><i class="bi bi-pencil me-2"></i>Edit Penawaran — {{ $quotation->quotation_number }}</div>
    <div class="card-body">
        <form method="POST" action="{{ route('sales.quotations.update', $quotation) }}">
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-4">
                    ...
                    <select name="customer_id" class="form-select" required>
                        ...
                        @foreach($customers as $c)<option value="{{ $c->id }}" @selected($quotation->customer_id == $c->id)>{{ $c->name }}</option>@endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Tanggal <span class="text-danger">*</span></label>
                    <input type="date" name="date" class="form-control" value="{{ $quotation->date->format('Y-m-d') }}" required>
                </div>
                <div class="col-md-3">
                        ...
                        <tr>
                            ...
                            <td colspan="2">
                                <div class="d-flex align-items-center gap-2">
                                    <input type="checkbox" id="taxCheck" class="form-check-input" @checked($quotation->tax_amount > 0) onchange="toggleTax()">
                                    <input type="number" name="tax_amount" id="taxAmount" class="form-control form-control-sm text-end" value="{{ $quotation->tax_amount }}" readonly>
                                </div>
                            </td>
                            <td></td>
                        </tr>
                        <tr class="fw-bold"><td colspan="3" class="text-end">Total</td><td colspan="2" class="text-end" id="grandTotal">Rp {{ number_format($quotation->total,0,',','.') }}</td><td></td></tr>
                    </tfoot>
                </table>
            </div>
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('sales.quotations.show', $quotation) }}" class="btn btn-secondary">Batal</a>
            </div>
        </form>
    </div>
</div>
