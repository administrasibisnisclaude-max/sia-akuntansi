        ...
        <form method="POST" action="{{ route('sales.invoices.update', $invoice) }}">
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-4">
                    ...
                    <select name="customer_id" class="form-select" required>
                        ...
                        @foreach($customers as $c)<option value="{{ $c->id }}" @selected($invoice->customer_id == $c->id)>{{ $c->name }}</option>@endforeach
                    </select>
                </div>
                <div class="col-md-3">
            ...
            <div class="table-responsive mb-3">
                <table class="table table-bordered">
                    ...
                    <tfoot>
                        <tr><td colspan="3" class="text-end fw-bold">Subtotal</td><td colspan="2" class="text-end" id="subTotal">Rp {{ number_format($invoice->subtotal,0,',','.') }}</td><td></td></tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td colspan="2">
                                <div class="d-flex align-items-center gap-2">
                                    <input type="checkbox" id="taxCheck" class="form-check-input" @checked($invoice->tax_amount > 0) onchange="toggleTax()">
                                    <input type="number" name="tax_amount" id="taxAmount" class="form-control form-control-sm text-end" value="{{ $invoice->tax_amount }}" readonly>
                                </div>
                            </td>
                            <td></td>
                        </tr>
                        <tr class="fw-bold"><td colspan="3" class="text-end">Total</td><td colspan="2" class="text-end" id="grandTotal">Rp {{ number_format($invoice->total,0,',','.') }}</td><td></td></tr>
                    </tfoot>
                </table>
            </div>
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('sales.invoices.show', $invoice) }}" class="btn btn-secondary">Batal</a>
            </div>
        </form>
    </div>
</div>
