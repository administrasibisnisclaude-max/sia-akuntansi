...
class ReportController extends Controller
{
    ...
    public function environmentalRecalc()
    {
        $sources = [
            [
        ...
        foreach ($sources as $src) {
            ...
            foreach ($lines as $line) {
                $exists = \App\Models\EnvironmentalImpact::where('reference_type', $src['ref_type'])
                    ->where('reference_id', $line->line_id)
                    ->exists();
                if ($exists) continue;

                \App\Models\EnvironmentalImpact::create([
        ...
        return redirect()->route('reports.environmental')
            ->with('success', "Berhasil menghitung ulang: {$inserted} data dampak lingkungan ditambahkan.");
    }
